...
ast copy_constructor(ast t)
{
    ...
    copy_param_list(param_list) ;
    copy_subr_body(subr_body) ;
...
ast copy_statements(ast t)
{
    ...
    for ( int i = 0 ; i < size ; i++ )
    {
        ...
        if ( deci != copy ) copied = true ;
        int t = (int)ast_node_kind(get_statement_statement(deci));
        if (copy != -1){
            decs.push_back(copy) ;
        }
        if ((int)ast_return_expr == t || t ==   (int)ast_return ){
...
ast copy_if(ast t)
{
    ...
    {
        ast expr = get_expr(condition,0);
        ast term = get_term_term(expr);
...
ast copy_while(ast t)
{
    ...
    if ( size_of_expr(condition) == 1 )
    {
        ast expr = get_expr(condition,0);
        ast term = get_term_term(expr);
        if( ast_node_kind(term) == ast_bool &&( get_bool_t_or_f(term) == false || size_of_statements(body) == 0) )
            return -1;
    }
    ast condition_copy = copy_expr(condition) ;
    ast body_copy = copy_statements(body) ;
...
ast copy_call_as_function(ast t)
{
    ...
    return create_call_as_function(class_name,subr_call_copy) ;
}
...
ast copy_call_as_method(ast t)
{
    ...
    return create_call_as_method(class_name,var_copy,subr_call_copy) ;
}
...
ast copy_subr_call(ast t)
{
    ...
    return create_subr_call(subr_name,copy) ;
}
