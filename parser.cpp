...
//
// NOTE: the best place to start is to write code to parse the input without atarrting to create
...
SEG LCL, THIS, ARG, STATIC;

void init_all_seg_name (){
    LCL.name = "local";
    THIS.name = "this";
    ARG.name = "argument";
    STATIC.name = "static";
...
void init (SEG * seg){
    seg->offset = 0;
    seg->ints = create_ints() ;
    seg->strings = create_strings() ;
}
void remove (SEG * seg){
    delete_ints(seg->ints) ;
    delete_strings(seg->strings) ;
}

void add (SEG * seg, string name, string type){
    insert_ints(seg->ints , name, seg->offset);
    insert_strings(seg->strings , name, type);
    seg->offset++;
}
...
ast find_var (string name){
    ...
    for (int i = 0; i < 4; i++){
        ...
        if ((offset = lookup_ints(p->ints,name)) != -1) {
            type = lookup_strings(p->strings,name);
            break;
        }
    }
    return offset != -1 ? create_var(name, p->name, offset, type) : -1;
}
...
// have(token, tk_identifier) ? get_identifier_name() : token_kind_to_string(token_kind(token));
string get_vtype_name(){
    Token token = parse_vtype();
    string res;
    if (have(token, tk_identifier)){
        res = get_identifier_name();
    }else { 
        res = token_kind_to_string(token_kind(token));
        mustbe(tk_vtype);
    }
    return res;
}
...
ast parse_class()
{
    ...
    mustbe(tk_class);
    class_name = get_identifier_name();
    mustbe(tk_lcb);
    ...
    pop_error_context() ;
    return create_class(class_name,class_var_decs,class_subrs);
}
...
ast parse_class_var_decs()
{
    ...
    {
        vector<ast> arr = have(tk_field) ? parse_field_var_dec() : parse_static_var_dec();
...
vector<ast> parse_static_var_dec()
{
    ...
    string name = get_identifier_name();
    decs.push_back( create_var_dec(name, STATIC.name, STATIC.offset, type) );
    add(&STATIC,name,type);
    while (have(tk_comma)){
        next_token();
        string name = get_identifier_name();
        decs.push_back( create_var_dec(name, STATIC.name, STATIC.offset, type) );
...
vector<ast> parse_field_var_dec()
{
    ...
    string type = get_type_name();
    string name = get_identifier_name();
    decs.push_back( create_var_dec(name, THIS.name, THIS.offset, type) );
    add(&THIS,name,type);
    while (have(tk_comma)){
        next_token();
        string name = get_identifier_name();
...
ast parse_subr_decs()
{
    ...
    while (have(tk_subroutine))
    {
        ...
        subrs.push_back(  create_subr( 
            have(tk_constructor) ? parse_constructor() : 
        (have(tk_method) ? parse_method() : parse_function())
        )  );
...
ast parse_param_list()
{
    ...
    if (have(tk_type))
    {
        string type = get_type_name();
        string name = get_identifier_name();
        params.push_back( create_var_dec(name, ARG.name, ARG.offset, type) );
        add(&ARG, name, type);
...
vector<ast> parse_var_dec()
{
    ...
    string type = get_type_name();
    string name = get_identifier_name();
    decs.push_back( create_var_dec(name, LCL.name, LCL.offset, type) );
    add(&LCL,name,type);
    while (have(tk_comma)){
        next_token();
        string name = get_identifier_name();
...
ast parse_let()
{
    ...
    pop_error_context() ;
    return index != -1 ? create_let_array(var,index,expr) : create_let(var,expr);
}
...
ast parse_if()
{
    ...
    pop_error_context() ;
    return if_false != -1 ? create_if_else(condition,if_true,if_false) : create_if(condition,if_true);
}
...
ast parse_do()
{
    ...
    ast res;
    if (have(tk_stop)){
        res = var != -1 ? create_call_as_method(get_var_type(var), var, parse_id_call())
         : create_call_as_function(name, parse_id_call());
    }else {
        res = create_call_as_method(class_name, create_this(), create_subr_call(name,parse_call())); 
    }
    mustbe(tk_semi);

    pop_error_context() ;
    return create_do(res) ;
}
...
ast parse_term()
{
    ...
    case tk_lrb:
        ...
        mustbe(tk_rrb);
        break ;
    case tk_not: case tk_sub:
        token = parse_unary_op();
        op = token_kind_to_string(token_kind(token));
        term = parse_term();
        term = create_unary_op(op, term);
        break ;
    case tk_identifier:
        term = parse_var_term();
        break;
...
ast parse_var_term()
{
    ...
    switch(token_kind(current_token())){
        ...
        break;
        case tk_lsb:
        res = create_array_index(var,parse_index());
        break;
        case tk_lrb:
        res = create_call_as_method(class_name, create_this(), create_subr_call(name,parse_call())); 
        break;
